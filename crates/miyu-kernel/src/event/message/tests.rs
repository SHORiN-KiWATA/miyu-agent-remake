...
//! 被打断的多写一格，没被打断的不写；坏的报错说清是哪一种。

use crate::block::Block;
use crate::event::{Body, Event};
use crate::test_support::{event_line, read_body, rejected};

const ASSISTANT: &str = r#"{"blocks":[{"type":"text","text":"我先看一下目录。"},{"type":"tool_call","call_id":"call_44_1","name":"read","args":"{\"path\":\"src\"}"}]}"#;
...
fn interrupted_is_written_only_when_true() {
    let cut = r#"{"blocks":[{"type":"text","text":"我先看"}],"interrupted":true}"#;
    ...
    let line = event_line("message.assistant", r#"{"blocks":[],"interrupted":false}"#);
    assert_eq!(
        Event::from_line(&line).unwrap().to_line(),
        event_line("message.assistant", r#"{"blocks":[]}"#)
    );
}
